...
export class ShipmentRepository {
    ...
    public async createShipment(shipment: IShipmentsResponse): Promise<void> {
        try {
            ...
            try {
                ...
                if (err && err.code === 11000 && err.keyPattern && err.keyPattern.trackingNumber) {
                    ...
                    for (let i = 0; i < maxRetries && !saved; i++) {
                        ...
                        try {
                            ...
                            if (!(e && e.code === 11000 && e.keyPattern && e.keyPattern.trackingNumber)) throw e;
                        }
                    }
                    if (!saved) throw new Error('Failed to save shipment: trackingNumber collisions');
                } else {
                    throw err;
                }
            }
    ...
    public async updateShipment(trackingId: string, shipment: IShipmentUpdate): Promise<void> {
        try {
